...
export default function ProviderDashboard() {
  ...
  const handleAccept = async (reqId: string) => {
    setAccepting(reqId);
    try {
      console.log("Aceptando solicitud:", reqId, "Proveedor:", user?.uid);
      await acceptServiceRequest(
        reqId, 
        user!.uid, 
        profile?.displayName || "Especialista",
        (profile as any)?.phone || "",
        "A la brevedad"
      );
      console.log("Solicitud aceptada correctamente");
    } catch (e: any) {
      console.error("Error en handleAccept:", e);
      alert(`Error al aceptar solicitud: ${e.message || "Error desconocido"}`);
    } finally {
      setAccepting(null);
    }
  };
  ...
  const getWhatsAppLink = (req: ServiceRequest) => {
    const phone = req.clientPhone || "";
    const name = req.clientName || "Cliente";
    const category = req.serviceCategory || "servicio";
    const message = encodeURIComponent(`Hola ${name}, soy ${profile?.displayName} de MaitenConnect respecto a tu solicitud de ${category}. ¿Cuándo te acomoda que coordinemos?`);
                  ...
                  <div key={req.id} className="bg-white dark:bg-slate-800 p-4 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                     <div>
                       <div className="flex items-center gap-2">
                         ...
                         <span className="text-[10px] bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-slate-400">ID: {req.id?.slice(-4)}</span>
                       </div>
                       <p className="text-foreground font-medium mt-1">"{req.description}"</p>
                       <p className="text-xs text-slate-400 mt-1">Solicitado por: <span className="font-semibold text-slate-500">{req.clientName || "Cliente"}</span></p>
                     </div>
                     <button 
                       onClick={() => handleAccept(req.id!)}
                       disabled={accepting === req.id}
                       className="w-full sm:w-auto bg-primary hover:bg-primary-dark disabled:bg-slate-400 text-white px-6 py-2 rounded-lg font-bold transition-colors shadow-sm"
                     >
                        {accepting === req.id ? "Aceptando..." : "Aceptar Trabajo"}
                     </button>
                  </div>
                ))
              )}
          ...
          <div className="glass-card p-6">
            ...
            <div className="space-y-4">
              {activeReqs.length === 0 ? (
                ...
                activeReqs.map(req => (
                  <div key={req.id} className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/50 p-4 rounded-xl flex flex-col gap-3 transition-all hover:shadow-md">
                    <div className="flex justify-between items-start">
                       <div>
                         ...
                         <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                           Cliente: <span className="font-bold">{req.clientName}</span>
                         </p>
                       </div>
                    ...
                    <div className="border-t border-emerald-200/50 dark:border-emerald-800/50 pt-3">
                       <a 
                         ...
                         className="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#1DA851] text-white py-2.5 rounded-lg font-bold transition-all shadow-sm hover:scale-[1.01]"
                       >
          ...
          <div className="glass-card p-6 bg-slate-50/50 dark:bg-slate-800/30">
            ...
            <div className="space-y-4 text-sm">
              ...
              <button 
                ...
                className="mt-4 w-full bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 text-foreground py-2 rounded-lg font-medium transition-colors shadow-sm"
              >
